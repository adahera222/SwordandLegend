...
function Start () {
	InvokeRepeating ("EnemyMove", 2, 2.7);
...
function Update () {
	...
	if (player.GetComponent(playerScript).playerHealth >= 70)
    {
    	visionLight.light.intensity = 0.0;
    }
    else if (player.GetComponent(playerScript).playerHealth >= 60)
    {
    	visionLight.light.intensity = 0.2;
    }
    else if (player.GetComponent(playerScript).playerHealth >= 50)
    {
    	visionLight.light.intensity = 0.4;
    }
    else if (player.GetComponent(playerScript).playerHealth >= 40)
    {
    	visionLight.light.intensity = 0.6;
    }
    else if (player.GetComponent(playerScript).playerHealth >= 30)
    {
    	visionLight.light.intensity = 0.7;
    }
    else if (player.GetComponent(playerScript).playerHealth >= 20)
    {
    	visionLight.light.intensity = 0.9;
    }
    else if (player.GetComponent(playerScript).playerHealth > 0)
    {
...
function OnGUI () {
    ...
    GUI.Box (Rect (12,188,130,20), "Player Health: "+player.GetComponent(playerScript).playerHealth+"%");
	GUI.DrawTexture(Rect(03,205,224,36), playerHealthBorder);
	var adjust : int = player.GetComponent(playerScript).playerHealth * 2;
	GUI.BeginGroup(Rect(15, 215,adjust,15));
	if(player.GetComponent(playerScript).playerHealth > 30)
	{
	...
	GUI.Box (Rect (12,248,130,20), "Stamina: "+ player.GetComponent(playerScript).playerStamina + "%");
	GUI.DrawTexture(Rect(03,265,224,36), playerStaminaBorder);
	GUI.BeginGroup(Rect(15, 275, player.GetComponent(playerScript).playerStamina*2,15));
	...
	GUI.Box (Rect (412,18,135,20), "Enemy Health: "+ enemy.GetComponent(enemyScript).enemyHealth + "%");
	GUI.DrawTexture(Rect(403,35,224,36), enemyHealthBorder);
	var adjust2 : int = enemy.GetComponent(enemyScript).enemyHealth * 2;
