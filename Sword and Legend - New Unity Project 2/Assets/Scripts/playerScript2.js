...
{
	isIdle = false;
	if((state == 0) && (isAttacking == false) && (playerStamina >= 30) && (isBlocking == false) && (isDodging == false) && (enemy.GetComponent(enemyScript2).enemyHealth > 0) && (playerHealth > 0))
	{
		isAttacking = true;
		loseAttackStamina();
        ...
        if((enemyBlock >= 6) && (enemy.GetComponent(enemyScript2).isInState == false))
        {
        	enemy.GetComponent(enemyScript2).blockedAttack();
        }
        else if(enemy.GetComponent(enemyScript2).isBlocking == false)
        {
        	//enemy.GetComponent(enemyScript2).loseHealth(15);
        }
	}
	else if((state == 0) && (isAttacking == false) && (playerStamina < 30) && (isBlocking == false) && (isDodging == false) && (enemy.GetComponent(enemyScript2).enemyHealth > 0) && (playerHealth > 0))
	{
		isAttacking = true;
		playerStamina = 0;
		player.animation.Play("player_weakend_attack");
        var enemyB = 10 * (Random.value);
        if((enemyB >= 2) && (enemy.GetComponent(enemyScript2).isInState == false))
        {
        	enemy.GetComponent(enemyScript2).blockedAttack();
        }
        else if(enemy.GetComponent(enemyScript2).isBlocking == false)
        {
        	//enemy.GetComponent(enemyScript2).loseHealth(5);
        }
	}
...
{
	isIdle = false;
	if(game.GetComponent(NextLevel).playerWin == false && game.GetComponent(NextLevel).gameOver == false)
	{
...
function loseHealth(damage : int)
{
	if(enemy.GetComponent(enemyScript2).enemyHealth > 0)
	{
		...
		{
			playerHealth = 0;
			game.GetComponent(NextLevel).lose();
		}
	}
}
...
function enemyPain()
{
	if(enemy.GetComponent(enemyScript2).isBlocking == false)
	{
		audio6.Play();
		enemy.GetComponent(enemyScript2).loseHealth(15);
	}
	...
	{
		audio1.Play();
		enemy.GetComponent(enemyScript2).blockSound();
	}
}
...
function enemyWeakPain()
{
	...
	{
		audio6.Play();
		enemy.GetComponent(enemyScript2).loseHealth(10);
	}
	else
	{
		audio1.Play();
...
{
	if(enemy.GetComponent(enemyScript2).isBlocking == true)
	{
		player.animation.Play("player_damaged");
	}
}
