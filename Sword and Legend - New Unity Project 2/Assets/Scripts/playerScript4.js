...
{
	isIdle = false;
	if((state == 0) && (isAttacking == false) && (playerStamina >= 30) && (isBlocking == false) && (isDodging == false) && (enemy.GetComponent(enemyScript4).enemyHealth > 0) && (playerHealth > 0))
	{
		isAttacking = true;
		loseAttackStamina();
        ...
        if((enemyBlock >= 0) && (enemy.GetComponent(enemyScript4).isBlocking == false))
        {
        	enemy.GetComponent(enemyScript4).blockedAttack();
        }
        else if(enemy.GetComponent(enemyScript4).isBlocking == false)
        {
        	//enemy.GetComponent(enemyScript).loseHealth(15);
        }
	}
	else if((state == 0) && (isAttacking == false) && (playerStamina < 30) && (isBlocking == false) && (isDodging == false) && (enemy.GetComponent(enemyScript4).enemyHealth > 0) && (playerHealth > 0))
	{
		isAttacking = true;
		playerStamina = 0;
		player.animation.Play("player_weakend_attack");
        var enemyB = 10 * (Random.value);
        if((enemyB >= 2) && (enemy.GetComponent(enemyScript4).isBlocking == false))
        {
        	enemy.GetComponent(enemyScript4).blockedAttack();
        }
        else if(enemy.GetComponent(enemyScript4).isBlocking == false)
...
{
	isIdle = false;
	if(game.GetComponent(Level4).playerWin == false && game.GetComponent(Level4).gameOver == false)
	{
...
function enemyPain()
{
	if(enemy.GetComponent(enemyScript4).isBlocking == false)
	{
		enemy.GetComponent(enemyScript4).loseHealth(15);
		audio6.Play();
	}
	else
	{
...
function enemyWeakPain()
{
	...
	{
		audio6.Play();
		enemy.GetComponent(enemyScript4).loseHealth(5);
	}
	else
	{
		audio1.Play();
...
{
	if(enemy.GetComponent(enemyScript4).isBlocking == true)
	{
		player.animation.Play("player_damaged");
	}
}
